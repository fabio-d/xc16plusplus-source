              ...
              register union internal_auxent *aux)
{
  ...
  /* the Microchip dsPIC (pic30) architecture does some Harvard architecture
     strangeness, and that's represented in the debug tools by locating data
     memory at 0x80000000 and program memory at everything under that. The
     symbol tables coming in, however, don't have that done yet, as everthing 
     except the debuggers works fine with the actual addresses.
  */
#ifdef PIC30_TARGET
  switch (cs->c_sclass)
    {
    ...
    case C_STAT:
      ...
      //    case C_EFCN:
      if (cs->c_secnum != 0
          && cs_to_section (cs, current_objfile) != SECT_OFF_TEXT)
        cs->c_value |= 0x80000000;
      break;
    default:
      break;
    }
