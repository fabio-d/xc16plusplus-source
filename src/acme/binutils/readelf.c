  ...
  */
  {
    ...
    for (j = 0; j < sec->sh_size; ++j)
    {
	((char *)debug_loc_contents)[j/2] = debug_loc_contents[j++];
    }
  }
#endif
}
  ...
  */
  {
    ...
    for (j = 0; j < sec->sh_size; ++j)
    {
	((char *)debug_str_contents)[j/2] = debug_str_contents[j++];
    }
  }
#endif
}
...
#if defined(PIC30)
		loc = section_begin + rp->r_offset / 2;
#else
