     ...
     asymbol **symbol_table;
{
...
#if defined(PIC30)
      /*
      ** This function's name implies that we should use the
      ** size after relocation.
      */
      bfd_size_type size;
      if (sec->reloc_done)
      {
      	size = bfd_get_section_size_after_reloc (sec);
      }
      else
      {
  ...
  */
  if (sec->reloc_done)
  {
    link_order.size = bfd_get_section_size_after_reloc (sec);
  }
  else
  {
    link_order.size = bfd_get_section_size_before_reloc (sec);
  }
#else
