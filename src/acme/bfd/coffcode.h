     ...
     flagword sec_flags;
{
  ...
  else if ((sec_flags & SEC_CODE) || (sec->auxflash == 1))
    {
      styp_flags = STYP_TEXT;
    }
     ...
     flagword sec_flags;
{
  ...
  /* READONLY later */
  /* skip RELOC */
  if (((sec_flags & SEC_CODE) != 0) || (sec->auxflash == 1))
  ...
  if ((sec_flags & SEC_CODE) || (sec->auxflash ==1))
     ...
     PTR filehdr;
{
  ...
  switch (internal_f->f_magic)
    ...
    {
      arch = bfd_arch_pic30;
      /* use the machine setting that was loaded from
         the input file by pic30_mk_object_hook() */
      if (abfd->arch_info)
        machine = abfd->arch_info->mach;
      break;
    }
#endif
...
*/
      ...
      if (current->persistent)
        section.s_flags |= STYP_PERSIST;
      else if (current->psv)
        section.s_flags |= STYP_PSV;
      else if (current->eedata)
        section.s_flags |= STYP_EEDATA;
      if (current->memory)
        section.s_flags |= STYP_MEMORY;

      /* modifiers can be combined */
      if (current->near)
        section.s_flags |= STYP_NEAR;
      if (current->xmemory)
        section.s_flags |= STYP_XMEM;
      if (current->ymemory)
        section.s_flags |= STYP_YMEM;
      if (current->absolute)
        section.s_flags |= STYP_ABSOLUTE;
      if (current->reverse)
        section.s_flags |= STYP_REVERSE;
/*      if (current->heap)
        section.s_flags |= STYP_HEAP; */
      if (current->dma)
        section.s_flags |= STYP_DMA;
      if (current->boot)
...
#endif
#if PIC30MAGIC
    /* For PIC30 the optional header is always present;
       processor ID is stored in the magic number field */
    internal_f.f_opthdr = bfd_coff_aoutsz (abfd);
#endif
    }
...
#define __A_MAGIC_SET__
...
#ifdef PIC30MAGIC
    {
      ...
      if (global_PROCESSOR)
         internal_a.magic = global_PROCESSOR->mach;
      else
         internal_a.magic = 0;
    }
#define __A_MAGIC_SET__
...
#undef __A_MAGIC_SET__
  ...
  if (abfd->flags & EXEC_P)
	...
	return FALSE;
      ...
      coff_swap_aouthdr_out (abfd, (PTR) & internal_a, (PTR) buff);
      amount = bfd_bwrite ((PTR) buff, bfd_coff_aoutsz (abfd), abfd);
		...
		dst->symbol.flags = BSF_DEBUGGING | BSF_DEBUGGING_RELOC;
		 ...
		 section.  */
	      ...
	      dst->symbol.value = (src->u.syment.n_value
				   - dst->symbol.section->vma);
#ifdef PIC30
              if (src->u.syment.n_sclass == C_EFCN)
                /* suppress these labels in disassembly listings */
                dst->symbol.flags |= BSF_DEBUGGING;
#endif
#endif
	      break;
...
#if 0
  printf("coff_slurp_reloc_table: %s %lx %lx\n",
         asect->name, asect->vma, asect->lma);
#endif
