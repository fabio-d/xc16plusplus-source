...
lang_set_startof ()
{
  ...
  for (s = output_bfd->sections; s != NULL; s = s->next)
	...
	{
          bfd_vma value;
	  value = bfd_get_section_vma (output_bfd, s);
          h->type = bfd_link_hash_defined;
          h->u.def.value = value & 0xFFFF;
	  h->u.def.section = bfd_abs_section_ptr;
	}

      sprintf (buf, ".startof_hi.%s", secname);
#if PIC30
      (void) strtok(buf,"%"); /* insert null at the first occurrence of % */
#endif
      h = bfd_link_hash_lookup (link_info.hash, buf, FALSE, FALSE, TRUE);
      ...
      if (h != NULL && h->type == bfd_link_hash_undefined)
	{
	  ...
	  h->type = bfd_link_hash_defined;
	  if (s->_cooked_size != 0)
	    value = s->_cooked_size / opb;
	  else
	    value = s->_raw_size / opb;
          h->u.def.value = value & 0xFFFF;
	  h->u.def.section = bfd_abs_section_ptr;
	}

      sprintf (buf, ".sizeof_hi.%s", secname);
#if PIC30
      (void) strtok(buf,"%"); /* insert null at the first occurrence of % */
#endif
      h = bfd_link_hash_lookup (link_info.hash, buf, FALSE, FALSE, TRUE);
      ...
      if (h != NULL && h->type == bfd_link_hash_undefined)
        {
          ...
          if (s->_cooked_size != 0)
            value = s->_cooked_size / opb +
                             bfd_get_section_vma (output_bfd, s);
          else
            value = s->_raw_size / opb + 
                             bfd_get_section_vma (output_bfd, s);
          h->u.def.value = value & 0xFFFF;
      ...
      sprintf (buf, ".endof_hi.%s", secname);
#if PIC30
      (void) strtok(buf,"%"); /* insert null at the first occurrence of % */
#endif
      h = bfd_link_hash_lookup (link_info.hash, buf, FALSE, FALSE, TRUE);
...
/* CAW - moved to the emulation file */
  ...
  /* Fix any .startof. ,.sizeof. or ,endof. symbols.  */
  lang_set_startof ();
